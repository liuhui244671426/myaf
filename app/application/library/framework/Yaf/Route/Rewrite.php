...
class Yaf_Route_Rewrite implements Yaf_Route_Interface
{
    ...
    protected function _match($path)
    {
        ...
        if ($this->_route !== '') {
            foreach (
                explode(
                    Yaf_Router::URI_DELIMITER, trim(
                        $this->_route, Yaf_Router::URI_DELIMITER
                    )
                ) as $pos => $part
            ) {
                if (
                    substr($part, 0, 1) == Yaf_Router::URI_VARIABLE
                    && substr($part, 1, 1) != Yaf_Router::URI_VARIABLE
                ) {
                    $name = substr($part, 1);
                    $parts[$pos] = null;
                    $variables[$pos] = $name;
        ...
        if ($path !== '') {
            ...
            foreach ($pathMatch as $pos => $pathPart) {
                ...
                if ($parts[$pos] == '*') {
                    ...
                    $matchedPath = implode(
                        Yaf_Router::URI_DELIMITER, $pathMatch
                    );
                    break;
                }
